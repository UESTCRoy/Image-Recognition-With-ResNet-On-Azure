...
def main(batch_size, epochs):
    ...
    full_dataset = MyDataset(train_label_file, 'train')
    train_num = len(full_dataset)

    #Set the validation ratio, 90% for train, 10% for test
    val_ratio = 0.1
    val_num = int(train_num * val_ratio)
    train_num = train_num - val_num

    # Split the dataset into train and validation datasets
    train_dataset, validate_dataset = torch.utils.data.random_split(full_dataset, [train_num, val_num])
    ...
    test_dataset = MyDataset("test_files.txt", "test")
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=batch_size,
                                              shuffle=False,
                                              num_workers=4)

    # Predict test labels and save to file
...
def create_test_file_list(test_files="test_files.txt"):
